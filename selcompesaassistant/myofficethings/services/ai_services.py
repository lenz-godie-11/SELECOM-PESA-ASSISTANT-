# Introduction to Selcom Pesa Customer Support Assistant
#
# This module demonstrates how to build a customer support assistant for Selcom Pesa using PydanticAI with DeepSeek integration, optimized for faster response times and async compatibility.
#
# Key features:
# - Language detection for English and Kiswahili
# - Conversation memory management
# - Query classification for Selcom-related topics
# - Structured responses using Pydantic models
# - Knowledge base integration with tool support
# - Regex-based response formatting
# - Self-correction with retries
# - Optimized for low-latency responses in async environments
...
client = OpenAI(
    api_key="sk-e5a9973a512644148e4f862537063f66",
...
model = OpenAIModel(
    'deepseek-chat',
    provider=DeepSeekProvider(api_key="sk-e5a9973a512644148e4f862537063f66")
)
...
# ------------------------------------------------------------------------------
#
# This section handles async loading and caching of the knowledge base for both English and Kiswahili.
#
# Key concepts:
# - Asynchronous file I/O for faster startup
...
async def load_knowledge():
    ...
    try:
        async with aio_open(os.path.join(knowledge_dir, "Knowledge.txt"), "r", encoding="utf-8") as file:
            english_knowledge = await file.read()
        
        async with aio_open(os.path.join(knowledge_dir, "kiswahiliKnowledge.txt"), "r", encoding="utf-8") as file:
            kiswahili_knowledge = await file.read()
        
        _KNOWLEDGE_CACHE = (english_knowledge, kiswahili_knowledge)
        return _KNOWLEDGE_CACHE
...
async def ensure_knowledge_loaded():
    global ENGLISH_KNOWLEDGE, KISWAHILI_KNOWLEDGE
...
# ------------------------------------------------------------------------------
# 2. Language Detection and Conversation Management
# ------------------------------------------------------------------------------
#
# This section handles language detection and conversation memory management.
#
# Key concepts:
# - Detecting Kiswahili vs English queries
# - Structured conversation history for efficiency
# - Managing context for follow-up questions
...
# ------------------------------------------------------------------------------
# 3. Agent with Query Classification
# ------------------------------------------------------------------------------
#
# This section defines an agent for classifying whether a query is Selcom-related.
#
# Key concepts:
# - Structured output with QueryClassification
# - Handling greetings specially
...
# ------------------------------------------------------------------------------
# 4. Agent with Structured Response
# ------------------------------------------------------------------------------
#
# This section defines an agent for generating structured responses.
#
# Key concepts:
# - Structured output with KnowledgeBasedResponse
# - Dynamic system prompt with conversation context
# - Tool for knowledge base lookup
...
response_agent = Agent(
    ...
    system_prompt=(
        ...
        "Use the knowledge base and conversation context to answer questions conversationally. "
    ...
    tools=[Tool(lambda ctx: ENGLISH_KNOWLEDGE if ctx.deps.get('language', 'en') == "en" else KISWAHILI_KNOWLEDGE, takes_ctx=True, name="get_knowledge_base")]
)
...
def validate_response(response: str) -> str:
    ...
    try:
        for pattern, replacement in FORMATTING_PATTERNS[:3]:  # Apply markdown, link fixes
        ...
        if FORMATTING_PATTERNS[3][0].search(response):  # Apply step formatting
...
async def get_structured_ai_response(user_id, user_input):
    """Get a structured, knowledge-based response using Pydantic AI."""
    ...
    try:
        ...
        response = await response_agent.run(user_prompt=user_input, deps=deps)
...
# ------------------------------------------------------------------------------
# 5. Agent with Direct AI Response
# ------------------------------------------------------------------------------
#
# This section provides a direct OpenAI response with regex-based formatting for compatibility.
#
# Key concepts:
# - Direct API interaction with connection pooling
# - Response formatting with regex
# - Performance timing
# - Context management
...
async def get_ai_response(user_id, user_input):
    ...
    try:
        ...
        knowledge_base = KISWAHILI_KNOWLEDGE if use_kiswahili else ENGLISH_KNOWLEDGE
